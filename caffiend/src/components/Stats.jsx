...
function StatCard(props){
    ...
    return(
        <div className={'card stat-card ' + (lg ? 'col-soan-2':'')}>
            <h4>{title}</h4>
            {children}
        </div>
    )
    ...
    return(
        ...
        <div className="stats-grid">
            ...
            <StatCard title="Daily Caffeine">
                <p><span className="stat-text"stat-text>{stats.daily_caffeine}</span>mg</p>
            </StatCard>

            <StatCard title="Avg # of Coffees">
            <p><span className="stat-text"stat-text>{stats.average_coffees}</span></p>
            </StatCard>
            <StatCard title="Daily Cost ($)">
            <p>$ <span className="stat-text"stat-text>{stats.daily_cost}</span></p>
            </StatCard>
            <StatCard title="Total Cost ($)">
            <p>$ <span className="stat-text"stat-text>{stats.total_cost}</span></p>
            </StatCard>
